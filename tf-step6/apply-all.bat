...
set "SECRET_JSON_FILE=%TEMP%\de-ai-07-rds-secret-%RANDOM%.json"
set "DB_ENV_FILE=%TEMP%\de-ai-07-rds-env-%RANDOM%.txt"
...
echo ============================================================
aws secretsmanager get-secret-value --region "%AWS_REGION%" --secret-id "%RDS_SECRET_ARN%" --query SecretString --output text > "%SECRET_JSON_FILE%"
if errorlevel 1 goto :error
...
if errorlevel 1 goto :error
kubectl create secret generic rds-secret --namespace "%APP_NAMESPACE%" --from-env-file="%DB_ENV_FILE%" --dry-run=client -o yaml | kubectl apply -f -
if errorlevel 1 goto :error
