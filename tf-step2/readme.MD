...
- 외부 입력하여 수정2
    - 변수 2개 추가
        ```
            ...
            instance_count = 1
        ```
    - terraform.tfvars, dev.tfvars, prod.tfvars 3개 준비
    ```powershell
    $path = "C:\Users\NT551_11TH\OneDrive\Desktop\workspace\tf_project\tf-step2"; New-Item -ItemType Directory -Path $path -Force; 'terraform.tfvars', 'dev.tfvars', 'prod.tfvars' | ForEach-Object { New-Item -ItemType File -Path (Join-Path $path $_) -Force }
    ```
...
- 사용 : resource 내부
- ex)
    - 보안 그룹에서 ingress 여러 포트를 오픈할 때 유용
    - 실제 구성은 web-was-db의 보안 그룹을 각각 구성할 때 진행
```
...
resource "aws_security_group" "DE-AI-25-IaC-TF-GROUP" {
    dynamic "ingress" {
        ...
        content {
            ...
            # dynamic "ingress" 이렇게 선언 했으므로 값은 ingress.value로 사용
...
# state (리소스 상태 조회)
- apply  생성 이후 내용에 대한 조회
```
# 새로 생성된 리소스 목록 출력
terraform state list
---
aws_eip.DE-AI-07-IaC-TF-EIP
aws_instance.DE-AI-07-IaC-TF-EC2
aws_security_group.DE-AI-07-IaC-TF-SG

# 상세 조회 (리소스가 만들어진후 확인 가능하다 plan보이는 내용들)
# terraform state show 리소스
terraform state show aws_instance.DE-AI-07-IaC-TF-EC2
---
# aws_instance.DE-AI-07-IaC-TF-EC2:
resource "aws_instance" "DE-AI-07-IaC-TF-EC2" {
    ami                                  = "ami-01e99d46e612f3624"
    arn                                  = "arn:aws:ec2:ap-northeast-2:827913617635:instance/i-06cca307a4999d54a"
    associate_public_ip_address          = true
    availability_zone                    = "ap-northeast-2c"
    disable_api_stop                     = false  <- 생성 이후 세팅된 값들
    ...
    primary_network_interface_id         = "eni-0eff638cd47e73c41"
    private_dns                          = "ip-172-31-45-87.ap-northeast-2.compute.internal"
    private_ip                           = "172.31.45.87"
    public_dns                           = "ec2-13-124-172-214.ap-northeast-2.compute.amazonaws.com"
    public_ip                            = "13.124.172.214"
    ...
    subnet_id                            = "subnet-049c8f72bb109a11e"
    tags                                 = {
    ...
    primary_network_interface {
        ...
        network_interface_id  = "eni-0eff638cd47e73c41"
    }
    ...
    root_block_device {
        ...
        tags_all              = {}
        throughput            = 125
        volume_id             = "vol-0db3bba5af41cb419"
        volume_size           = 2
        volume_type           = "gp3"
    }
}
